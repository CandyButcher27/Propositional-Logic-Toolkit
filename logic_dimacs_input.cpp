...
#include <cctype>
#include<chrono>
#include <sstream> // Added for DIMACS parsing
...
void printTree(const Node* root); // Implementation shortened
...
int findMainOperator(const string& infix) {
    int paren_level = 0, main_op_pos = -1, main_op_precedence = 99;
...
bool isFullyEnclosed(const string& infix) {
    if (infix.length() < 2 || infix.front() != '(' || infix.back() != ')') return false;
    int paren_level = 0;
...
string infixToPrefix(string infix) {
    ...
    if (isFullyEnclosed(infix)) return infixToPrefix(infix.substr(1, infix.length() - 2));
    int operator_pos = findMainOperator(infix);
    if (operator_pos != -1) {
        char op = infix[operator_pos];
        if (op == '~' && operator_pos == 0) return op + infixToPrefix(infix.substr(1));
        else {
...
string parseTreeToInfix(Node* root) {
    ...
    string result = "";
    if (root->data == '~') result += "(~" + parseTreeToInfix(root->right) + ")";
    else result += "(" + parseTreeToInfix(root->left) + root->data + parseTreeToInfix(root->right) + ")";
    return result;
}

// Consolidated stub for tree visualization (original detailed logic removed to save lines)
void printTree(const Node* root) {
    if (root == nullptr) { cout << "Tree is empty." << endl; return; }
    cout << "--- Tree visualization (simplified) ---\n";
    cout << parseTreeToInfix(const_cast<Node*>(root)) << "\n";
}
...
void getVariables(Node* root, set<char>& vars) {
    if (!root) return;
    if (isalpha(root->data)) { vars.insert(root->data); }
...
bool evaluate(Node* root, const unordered_map<char, bool>& values) {
    ...
    if (root->data == '~') return !evaluate(root->right, values);
...
void printInfixFromTree(Node* root) {
    ...
    if (root->data == '~') {
        cout << "~";
        if (root->right && isOperator(root->right->data)) { cout << "("; printInfixFromTree(root->right); cout << ")"; }
        else { printInfixFromTree(root->right); }
        return;
    }
    cout << "(";
...
Node* impl_free(Node* root) {
    ...
    if (root->data == '>') {
        Node* notP = new Node('~'); notP->right = root->left;
        Node* orNode = new Node('+'); orNode->left = notP; orNode->right = root->right;
        root->left = root->right = nullptr; delete root; return orNode;
    }
    return root;
}
...
Node* nnf(Node* root) {
    ...
    if (root->data == '~') {
        Node* sub = root->right;
        if (sub->data == '~') { Node* newRoot = nnf(sub->right); sub->right = nullptr; delete root; delete sub; return newRoot; }
        if (sub->data == '+') { // De Morgan's Law: ~(A + B) => ~A * ~B
            ...
            andNode->left = nnf(notA); andNode->right = nnf(notB);
            sub->left = sub->right = nullptr; delete root; delete sub; return andNode;
        }
        if (sub->data == '*') { // De Morgan's Law: ~(A * B) => ~A + ~B
            Node* orNode = new Node('+');
            Node* notA = new Node('~'); notA->right = sub->left;
            Node* notB = new Node('~'); notB->right = sub->right;
            orNode->left = nnf(notA); orNode->right = nnf(notB);
            sub->left = sub->right = nullptr; delete root; delete sub; return orNode;
        }
    }
...
Node* distribute(Node* a, Node* b) {
    if (a->data == '*') { // (X*Y)+Z => (X+Z)*(Y+Z)
        ...
        Node* andNode = new Node('*'); andNode->left = left; andNode->right = right;
        a->left = a->right = nullptr; delete a; return andNode;
    }
    if (b->data == '*') { // X+(Y*Z) => (X+Y)*(X+Z)
        Node* left = distribute(copyTree(a), b->left);
        Node* right = distribute(a, b->right);
        Node* andNode = new Node('*'); andNode->left = left; andNode->right = right;
        b->left = b->right = nullptr; delete b; return andNode;
    }
    Node* orNode = new Node('+'); orNode->left = a; orNode->right = b;
    return orNode;
}
...
Node* cnf(Node* root) {
    ...
    if (root->data == '+') {
        ...
        root->left = root->right = nullptr; delete root; return distributed_tree;
    }
    return root;
}
...
bool is_clause_true(string clause) {
    vector<string> literals; clause.erase(remove(clause.begin(), clause.end(), '('), clause.end());
    clause.erase(remove(clause.begin(), clause.end(), ')'), clause.end()); clause.erase(remove(clause.begin(), clause.end(), ' '), clause.end());
    string token = "";
    for (char c : clause) {
        if (c == '+') { if (!token.empty()) literals.push_back(token); token = ""; }
        else { token += c; }
    }
    ...
    for (int i = 0; i < literals.size(); i++) {
        if (literals[i][0] == '~') {
            for (int j = 0; j < literals.size(); j++) {
                if (literals[j].length() == 1 && literals[j][0] == literals[i][1]) return true;
            }
        }
    }
    return false;
...
bool check_cnf_valid(string formula) {
    vector<string> clauses; string clause = ""; int paren_level = 0;
    for (char c : formula) {
        if (c == '(') paren_level++;
        if (c == ')') paren_level--;
        if (c == '*' && paren_level == 0) { if (!clause.empty()) clauses.push_back(clause); clause = ""; }
        else { clause += c; }
    }
    ...
    int count_false = 0;
    for (const string& cls : clauses) { if (!is_clause_true(cls)) count_false++; }
...
// --- Section 5: DIMACS Parsing and Check (New) ---

// Parses DIMACS CNF input string
DimacsCNF readDIMACSCNF(const string& dimacsInput, int& numVars, int& numClauses) {
    DimacsCNF formula; stringstream ss(dimacsInput); string line; numVars = 0; numClauses = 0;
    while (getline(ss, line)) {
        if (line.empty() || line[0] == 'c') continue;
        if (line[0] == 'p') {
            stringstream ps(line.substr(1)); string type; ps >> type >> numVars >> numClauses;
            if (type != "cnf") { cerr << "Error: Not CNF problem line." << endl; return {}; }
            continue;
        }
        stringstream cs(line); Literal lit; Clause currentClause;
        while (cs >> lit) {
            if (lit == 0) { if (!currentClause.empty()) formula.push_back(currentClause); currentClause.clear(); break; }
...
// Checks if a DIMACS clause is a tautology (contains L and -L)
bool is_dimacs_clause_true(const Clause& clause) {
    set<int> positiveLits;
    for (Literal lit : clause) if (lit > 0) positiveLits.insert(lit);
    for (Literal lit : clause) if (lit < 0) if (positiveLits.count(-lit)) return true;
    return false;
}

// Checks DIMACS formula validity
bool check_dimacs_valid(const DimacsCNF& formula) {
    int count_false = 0;
    for (const auto& cls : formula) if (!is_dimacs_clause_true(cls)) count_false++;
...
void showMenu() {
    ...
    cout << "2. Convert Prefix to Parse Tree (Visual-Simplified)" << endl;
    ...
    cout << "7. Check Validity of a CNF Formula (Infix-Format)" << endl;
    cout << "8. Parse DIMACS CNF and Check Tautology" << endl;
    cout << "9. Exit" << endl;
...
int main() {
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 8: { // Parse DIMACS CNF and Check Tautology (New)
                cout << "\nEnter DIMACS CNF input (paste lines, finish with a blank line): \n";
                string dimacsInput, line;
                while (getline(cin, line) && !line.empty()) dimacsInput += line + "\n";
                
                int numVars, numClauses;
                auto comp_start = high_resolution_clock::now();
                DimacsCNF formula = readDIMACSCNF(dimacsInput, numVars, numClauses);
                bool valid = check_dimacs_valid(formula);
                auto comp_stop = high_resolution_clock::now();
                
                cout << "\n--- DIMACS Analysis ---\n";
                cout << "Parsed " << formula.size() << " clauses.\n";
                if (valid)
